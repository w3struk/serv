...
./setup.sh add-modern-inbound # добавить inbound для постепенной миграции
...
./setup.sh add-modern-inbound # отдельный modern VLESS/XHTTP inbound и маршрут Caddy
...
### Постепенная миграция на modern inbound

После первоначальной установки можно создать второй, отдельный modern inbound и отдельный маршрут Caddy:

```bash
./setup.sh add-modern-inbound
```

#### Предварительные условия

- Первоначальная установка должна быть завершена и работать; legacy inbound и его маршрут Caddy должны оставаться доступными.
- Запускайте команду из каталога установки, обычно `/opt/serv`.
- Для настройки переносимых пользователей нужен доступ к UI 3x-ui и совместимый клиент.

Команда предназначена для поэтапной миграции. Она создаёт modern VLESS/XHTTP inbound и маршрут Caddy для него, но **не изменяет** legacy inbound, не создаёт клиентов и не переносит существующих клиентов. Поэтому все старые клиенты продолжают работать через legacy inbound, пока их не перенастроят вручную.

#### Что нужно сделать для каждого переносимого клиента

После успешного выполнения команды настройте каждого пользователя вручную в UI 3x-ui:

1. Создайте или отредактируйте клиента для modern inbound и прикрепите его к нему.
2. Сначала создайте и импортируйте modern-профиль, затем проверьте modern-подключение; legacy-профиль оставьте доступным до успешной проверки.
3. После успешной проверки отсоедините клиента от legacy inbound и не оставляйте его одновременно на обоих inbound.
4. Убедитесь, что в импортированном профиле включены обязательные **VLESS Encryption** и `flow: xtls-rprx-vision`, если они предоставлены сгенерированной конфигурацией.

Не переносите старые профили автоматически: они остаются профилями legacy inbound. Новому пользователю или перенесённому пользователю нужен профиль/подписка modern inbound.

#### Повторный запуск, резервная копия и откат

`add-modern-inbound` идемпотентна: повторный запуск не должен создавать ещё один modern inbound или ещё один маршрут Caddy. Перед изменениями команда сохраняет резервную копию SQLite и Caddyfile в защищённом root-owned каталоге. Резервная копия не используется для автоматического отката API/DB-изменений: сохраните её для ручного восстановления при необходимости. При ошибке команда автоматически восстанавливает только Caddyfile, если это безопасно; созданные inbound/Host не удаляются. Для справки используйте `./setup.sh help`.
...
- **Первоначальный inbound:** первоначальная установка создаёт `VLESS-XHTTP-Backend` (`network: "xhttp"`, `security: "none"`, UDS/h2c). Публичный TLS завершает Caddy.
...
`$XHTTP_PATH` — путь XHTTP legacy inbound, который генерирует первоначальный установщик. В текущем `setup.sh` это значение вида `api/vN`, поэтому публичный маршрут выглядит как `/api/vN/*`. Схема ниже описывает legacy inbound; modern inbound создаётся отдельно командой `add-modern-inbound`.
...
Вне `xhttpSettings` первоначальный установщик задаёт для legacy inbound:
